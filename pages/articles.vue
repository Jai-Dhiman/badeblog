<template>
  ...
  <div v-else class="text-center py-4">
    <p class="text-gray-600 mb-4">No articles category found.</p>
...
onMounted(async () => {
  try {
    ...
    const articleCategory = categories.find(
      (c: any) => c.name.toLowerCase() === 'article'
    )
    
    if (articleCategory) {
      categoryId.value = articleCategory.id
    } else {
      console.warn('Article category not found. Available categories:', availableCategories.value)
    }
  } catch (err) {
