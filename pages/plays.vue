<template>
  ...
  <div v-else class="text-center py-4">
    <p class="text-gray-600 mb-4">No plays category found.</p>
...
onMounted(async () => {
  try {
    ...
    const playsCategory = categories.find(
      (c: any) => c.name.toLowerCase() === 'play'
    )
    
    if (playsCategory) {
      categoryId.value = playsCategory.id
    } else {
      console.warn('Play category not found. Available categories:', availableCategories.value)
    }
  } catch (err) {
