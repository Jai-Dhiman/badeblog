...
const { subscribeToNewStories } = useSupabaseData()
...
const handleSubmit = async () => {
  ...
  message.value = ''
  try {
    await subscribeToNewStories(email.value)
    message.value = 'Successfully subscribed!'
    status.value = 'success'
    email.value = ''
  } catch (err) {
    const error = err as Error
    message.value = error.message || 'Failed to subscribe or Already Subscribed.'
    status.value = 'error'
  } finally {
    loading.value = false
  }
